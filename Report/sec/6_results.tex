...
\begin{enumerate}
  \item original images without preprocessing, 
  \item grayscale+blur preprocessed images, and 
  \item preprocessed images with edge extraction applied using the Canny-based edge extractor described above. 
\end{enumerate}
For all three datasets, the network architecture, optimization settings, and training schedule were kept identical 
to isolate the effect of the input representation on optimization behavior.

\subsubsection{Convergence Comparison}
Training and validation losses over 5 epochs were recorded for each dataset and plotted on a single curve for comparison. 
The original-image setting consistently produced the highest training and validation losses; applying basic preprocessing reduced both losses, 
while adding edge extraction on top of the preprocessing yielded the lowest losses and the steepest early decrease, 
indicating faster and more stable convergence. 
Therefore, subsequent experiments in the paper are conducted using this representation as the default input to the model.

\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{Images/convergence_comparison.png}
\caption{Comparison of convegence speeds when using datasets with different preprocessing steps for training.}
...
\begin{itemize}
    ...
    \item \textbf{Input}: $224 \times 224$ RGB → 196 patches
...
We also explore an alternative implementation of our framework using different CNN architectures in place of our vision transformer.
The architectures used and results are listed as follows.
...
This architecture utilizes a pretrained backbone, either EfficientNet ~\cite{EfficientNet} or ResNet ~\cite{ResNet}, to extract features. 
The extracted features are subsequently flattened and passed through linear layers, culminating in a regression layer that outputs 4 scores. 
A sigmoid activation function is applied prior to the final output to constrain the scores within the range of 0 to 1.

During evaluation on 800 images, this architecture achieved the best performance among all tested CNN models, attaining an average MSE loss of 0.0075. 
The final training loss was 0.0063, while the best validation loss reached 0.0251.

Given the limited scale of our dataset, comprising only 1,600 images, transfer learning proved to be the most effective approach, yielding optimal results. 
Specifically, the model was implemented using the ImageNet1K\_V1 pretrained weights, with torchvision.models.resnet18 serving as the backbone for the transfer learning procedure.
...
In our experiments, we use GPT-4o for the LLM.

\begin{figure}[t]
  \centering
   \includegraphics[width=1\linewidth]{Images/ui_palm_new.jpeg}
...
\begin{figure}[t]
  \centering
   \includegraphics[width=1\linewidth]{Images/ui_main_new.jpeg}

   \caption{Prediction by LLM.}
   \label{fig:onecol}
\end{figure}
