...
This architecture utilizes a pretrained backbone, either EfficientNet ~\cite{EfficientNet} or ResNet ~\cite{ResNet}, to extract features. The extracted features are subsequently flattened and passed through linear layers, culminating in a regression layer that outputs 4 scores. A sigmoid activation function is applied prior to the final output to constrain the scores within the range of 0 to 1.

During evaluation on 800 images, this architecture achieved the best performance among all tested CNN models, attaining an average MSE loss of 0.0075. The final training loss was 0.0063, while the best validation loss reached 0.0251.

Given the limited scale of our dataset, comprising only 1,600 images, transfer learning proved to be the most effective approach, yielding optimal results. Specifically, the model was implemented using the ImageNet1K_V1 pretrained weights, with torchvision.models.resnet18 serving as the backbone for the transfer learning procedure.
